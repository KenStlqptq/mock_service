...
var ByteBuffer = function (org_buf, offset) {
    ...
    this.readuint = function (len) {
        let tmp = _org_buf['readUInt' + _endian + 'E'](_offset, len);
        _offset += len;
        return tmp;
    };
    this.writeuint = function (val, len) {
        _list.push({ t: Type_UInt, d: val, l: len });
        _offset += len;
    };
    /**
     * 变长字符串 前2个字节表示字符串长度
    ...
    this.readbytearray = function (len) {
        if (!len) {
            throw new Error('byteArray must got len argument');
            return this;
        }
    ...
    this.writebytearray = function (val, len) {
        if (!len) {
            throw new Error('byteArray must got len argument');
            return this;
        }
        _list.push({ t: Type_ByteArray, d: val, l: len });
        _offset += len;
    };
    /**
     * 写入一个Long类型
     **/
    // this.long = function (val, index) {
    //     if (val == undefined || val == null) {
    //         var highNum = _org_buf['readUInt32' + _endian + 'E'](_offset);
    //         var lowNum = _org_buf['readUInt32' + _endian + 'E'](_offset + 4);
    //         var long = Long.fromBits(lowNum, highNum);
    //         _list.push(long);
    //         _offset += 8;
    //     } else {
    //         var long = Long.fromString(val, 10);
    //         _list.splice(index != undefined ? index : _list.length, 0, {
    //             t: Type_Long,
    //             d1: long.getHighBitsUnsigned(),
    //             d2: long.getLowBitsUnsigned(),
    //             l: 8
    //         });
    //         _offset += 8;
    //     }
        ...
        for (var i = 0; i < _list.length; i++) {
            switch (_list[i].t) {
                ...
                case Type_UInt32:
                    ...
                    break;
                case Type_UInt:
                    _org_buf['writeUInt' + _endian + 'E'](_list[i].d, offset, _list[i].l);
                    offset += _list[i].l;
                    break;
                // case Type_Long:
                //     _org_buf['writeUInt32' + _endian + 'E'](_list[i].d1, offset);
                //     _org_buf['writeUInt32' + _endian + 'E'](_list[i].d2, offset + 4);
                //     offset += _list[i].l;
                //     break;
                case Type_String:
                    //前2个字节表示字符串长度
                    ...
                    offset += 2;
                    _org_buf.write(_list[i].d, offset);
                    offset += _list[i].l;
                    break;
                case Type_VString:
...
ByteBuffer.create = function (bigEndian, msg) {
    if (bigEndian) {
        return new ByteBuffer(msg).bigEndian();
    }
    return new ByteBuffer(msg).littleEndian();
};
